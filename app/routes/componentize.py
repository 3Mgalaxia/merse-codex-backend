...
from app.utils.prompts import prompt_componentize
from app.utils.validate import safe_len

router = APIRouter()


@router.post("/componentize", response_model=EditResponse)
async def componentize(payload: EditRequest) -> EditResponse:
    html_len = safe_len(payload.html)
    comando_len = safe_len(payload.comando)

    modelo = escolher_modelo(
        task=CodexTask.COMPONENTIZE,
        html_len=html_len,
        comando_len=comando_len,
    )

    prompt = prompt_componentize(payload.html, payload.comando)

    try:
        resultado = CodexEngine.run(
            model=modelo,
            system_prompt="Você é o Merse Codex e cria componentes reutilizáveis a partir de HTML Merse.",
            user_prompt=prompt,
            temperature=0.2,
        )
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Erro ao componentizar HTML: {exc}")
